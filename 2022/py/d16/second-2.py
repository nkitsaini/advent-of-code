...
for v_name, vavls in valves.items():
    for tun in vavls[1]:
        reverse_tunnels[tun].append(v_name)

def is_bad(valve: str, visited = None) -> bool:
    if (visited ==None):
        visited = []
    if valve in visited:
        return True
    ...
    for nv in valves[valve][1]:
        if not is_bad(nv, visited):
            return False
...
for v_name in valves.keys():
    print(v_name, reverse_tunnels[v_name], valves[v_name][0], is_bad(v_name))
# pprint(reverse_tunnels)
exit(0)
@lru_cache(100000000000)
